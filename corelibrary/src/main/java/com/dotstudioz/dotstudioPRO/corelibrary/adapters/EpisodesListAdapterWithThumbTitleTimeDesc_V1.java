 ...
 */
public class EpisodesListAdapterWithThumbTitleTimeDesc_V1 extends BaseAdapter {
    ...
    public EpisodesListAdapterWithThumbTitleTimeDesc_V1(Activity activity, List<EpisodeItem> episodeItemsList) {
        this.activity = activity;
        this.episodeItemsList = episodeItemsList;

        if (activity instanceof EpisodesListAdapterWithThumbTitleTimeDesc_V1.IEpisodesListAdapter)
            iEpisodesListAdapter = (EpisodesListAdapterWithThumbTitleTimeDesc_V1.IEpisodesListAdapter) activity;
        else
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null) {
            convertView = inflater.inflate(R.layout.episodes_list_with_thumb_title__time_desc_item_view_v1, parent, false);
        ...
        final TextView metaInfoTextView = (TextView) convertView.findViewById(R.id.metaInfoTextView);
        metaInfoTextView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //iEpisodesListAdapter.playButtonClicked(positionToShare, viewToShare, parentToShare);
                if(metaInfoTextView.getMaxLines() == 2) {
                    metaInfoTextView.setMaxLines(Integer.MAX_VALUE);
                    metaInfoTextView.setEllipsize(null);
                } else {
                    metaInfoTextView.setMaxLines(2);
                    metaInfoTextView.setEllipsize(TextUtils.TruncateAt.END);
                }
            }
        });
        descLinearLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //iEpisodesListAdapter.playButtonClicked(positionToShare, viewToShare, parentToShare);
                if(metaInfoTextView.getMaxLines() == 2)
                    metaInfoTextView.setMaxLines(20);
                else
                    metaInfoTextView.setMaxLines(2);
            }
        });
        ...
        if(episodeItem.getVideoPausedPoint() > 0 &&
            ...
            /*convertView.findViewById(R.id.parentProgressContanier).setBackgroundColor(Color.parseColor("#D1D1D1"));
            convertView.findViewById(R.id.parentProgressContanier1).setBackgroundColor(Color.parseColor("#D1D1D1"));*/
            convertView.findViewById(R.id.parentProgressContanier).setBackgroundColor(Color.parseColor("#D1D1D1"));
            convertView.findViewById(R.id.parentProgressContanier1).setBackgroundColor(Color.parseColor("#D1D1D1"));
            convertView.findViewById(R.id.childProgressContainer).setBackgroundColor(Color.parseColor(activeColor));
            convertView.findViewById(R.id.childProgressContainer1).setBackgroundColor(Color.parseColor(activeColor));
            ...
            ((RelativeLayout.LayoutParams)convertView.findViewById(R.id.parentProgressContanier1).getLayoutParams()).height = heightForProgressBar;
            ((RelativeLayout.LayoutParams)convertView.findViewById(R.id.parentProgressContanier1).getLayoutParams()).width = imageWidth;
            ...
            int actualWidth1 = 0;
            actualWidth1 = ((int) TypedValue.applyDimension( TypedValue.COMPLEX_UNIT_DIP, 80, displayMetrics ));

            //videoPausedPointPercentage = 100;

            int widthOfProgress1 = 0;
            if(actualWidth1 <= 0)
                widthOfProgress1 = ((convertView.findViewById(R.id.seasonPlayImage).getMeasuredWidth()*videoPausedPointPercentage)/100);
            else
                widthOfProgress1 = ((imageWidth*videoPausedPointPercentage)/100);
        ...
        if(episodeItem.getVideoDescription() != null && episodeItem.getVideoDescription().length() > 0)
            metaInfoTextView.setText(episodeItem.getVideoDescription());
        ...
        if(isCustomFontEnabledForVideoTitle) {
            ...
            metaInfoTextView.setTypeface(customFontForVideoDescription);
        } else {
            episodeInfoTextView.setTypeface(FontsConstants.tfRegular);
            metaInfoTextView.setTypeface(FontsConstants.tfRegular);
        }
        ...
        if(episodeNameWithSeriesTitle != null && episodeNameWithSeriesTitle.length() > 0)
            seasonName.setText(episodeNameWithSeriesTitle);
        else
            seasonName.setText("Episode "+(position+1));
        ...
        if(imageString.indexOf("https://images.dotstudiopro.com/") == 0) {
