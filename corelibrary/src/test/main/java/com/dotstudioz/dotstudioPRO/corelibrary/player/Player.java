...
import com.dotstudioz.dotstudioPRO.dsplayer.player.hls.HlsEventProxy;
 ...
 */
public class Player implements ExoPlayer.Listener, HlsTrackSelector, HlsEventProxy.InfoListener, MediaCodecVideoTrackRenderer.EventListener {

    @Override
    public void selectTracks(HlsMasterPlaylist playlist, Output output) throws IOException {
        for (Listener listener : listeners) {
            listener.selectTracks(playlist, output);
        }
    }
    ...
    public void onVideoFormatEnabled(Format format, int trigger, long mediaTimeMs) {
        for (Listener listener : listeners) {
            listener.onVideoFormatEnabled(format, trigger, mediaTimeMs);
        }
    }
    ...
    public void onAudioFormatEnabled(Format format, int trigger, long mediaTimeMs) {
        for (Listener listener : listeners) {
            listener.onAudioFormatEnabled(format, trigger, mediaTimeMs);
        }
    }
    ...
    public void onLoadStarted(int sourceId, long length, int type, int trigger, Format format, long mediaStartTimeMs, long mediaEndTimeMs) {
        for (Listener listener : listeners) {
            listener.onLoadStarted(sourceId, length, type, trigger, format, mediaStartTimeMs,mediaEndTimeMs);
        }
    }
    ...
    public void onLoadCompleted(int sourceId, long bytesLoaded, int type, int trigger, Format format, long mediaStartTimeMs, long mediaEndTimeMs, long elapsedRealtimeMs, long loadDurationMs) {
        for (Listener listener : listeners) {
            listener.onLoadCompleted(sourceId, bytesLoaded, type, trigger, format, mediaStartTimeMs, mediaEndTimeMs, elapsedRealtimeMs, loadDurationMs);
        }
    }
    ...
    public interface Listener {
        ...
        void selectTracks(HlsMasterPlaylist playlist, Output output);
    ...
    public Player() {
        ...
        //player = ExoPlayer.Factory.newInstance(RENDERER_COUNT, 2500, 5000);
        player = ExoPlayer.Factory.newInstance(RENDERER_COUNT, 500, 1000);
    ...
    public void setMediaControllerForThePlayer(MediaController mediaController) {
        mediaController.setMediaPlayer(getPlayerControl());
        mediaController.setEnabled(true);
    }
    public void setMediaControllerForThePlayer(PlayControlControllerView playControlControllerView) {
        playControlControllerView.setMediaPlayer(getPlayerControl());
        playControlControllerView.setEnabled(true);
    }
