                    ...
                    imageView.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            if(channelIdPlusCategorySlugFromMultiViewPager != null && channelIdPlusCategorySlugFromMultiViewPager.length() > 0) {
                                ...
                                //System.out.println("indexOfChannelClicked==>" + indexOfChannelClicked);
                                //System.out.println("indexOfChannelClicked==>" + ApplicationConstants.selectedIndexOfChannel);

                                if(indexOfChannelClicked >= spotLightCategoriesDTO.getSpotLightChannelDTOList().size())
                                    indexOfChannelClicked = spotLightCategoriesDTO.getSpotLightChannelDTOList().size()-1;

                                iVideoOrChannelItemPagerAdapter.channelItemClickHandler(spotLightCategoriesDTO.getSpotLightChannelDTOList().get(indexOfChannelClicked).getId() + "|" + spotLightCategoriesDTO.getCategorySlug());

                                indexOfChannelClicked = 0;
                                ApplicationConstants.selectedIndexOfChannel = 0;
                            } else {
                                if(indexOfChannelClicked >= spotLightCategoriesDTO.getSpotLightChannelDTOList().size())
                                    indexOfChannelClicked = spotLightCategoriesDTO.getSpotLightChannelDTOList().size()-1;
